...
for i, sel_day in enumerate(all_days):
    print(f"==Trial: {trialname} Processing day: {sel_day}, day {i + 1} out of {total_days} days==")
    ...
    pbar = tqdm(enumerate(days[:]), total=total_test_days)
    for j, day in pbar:
        ...
        # c = time.time()
        # time_diff_ms = (c - a) * 1000
        # print(f"Preprocessing time: {time_diff_ms:.2f} ms")
